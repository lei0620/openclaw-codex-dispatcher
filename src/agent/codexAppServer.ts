...
async function ensureAppServer(
  ...
  onLog("system", `starting desktop app-server on ${appServer.url}`);
  const child = spawn(command, ["app-server", "--listen", appServer.url], {
    detached: true,
    windowsHide: true,
    stdio: "ignore"
  });
...
function resolveCodexAppServerCommand(configuredCommand?: string): string {
